...
const asset P1_BLOCKER_MODEL   =  $"models/robots/marvin/marvin.mdl" //$"models/vehicle/crow_dropship/crow_dropship.mdl" 
const asset P1_BLOCKER_HIDDEN_COLLISION = $"models/robots/marvin/marvin.mdl" //$"models/industrial/wall_tech_128.mdl"
...
//// P1 VECTOR STUFF ////
const array<vector> P1_BLOCK_POINTS_RIGHT = [ 
	<12435.83, 6429.00, 780.27>,
	<12536.27, 7146.89, 729.19>,
]

const array<vector> P1_BLOCK_POINTS_FORWARD = [
	<12353.66, 7488.32, 840.18>, 
	<11750.40, 7476.17, 840.18>,
	<11113.99, 7482.99, 840.18>,
]

/*
const array<vector> P1_BLOCK_RESET_POINTS = [ 
	<>,
	<>,
	<>, 
	<>,
...
void function CodeCallback_MapInit()
{
	...
	PrecacheModel( P1_BLOCKER_MODEL )
...
void function RainbowSky()
{
	SetConVarFloat( "mat_sky_scale", 0.1 )
    
	float r = 50
	float g = 100 
	float b = 200
	bool incrementR = true
	bool incrementG = true
	bool incrementB = true
	

	while ( true )
	{
		// should we flip inc/dec?
		if ( r <= 5 || r >= 255 )
		...
		// apply
		WaitPrecise( Time(), 0.01 )
		SetConVarString( "mat_sky_color", r + " " + g + " " + b + " 1" )
...
void function WaitPrecise( float startTime, float timer )    // hack: need this to be able to wait for < 0.1. this is VERY dumb.
{
...
void function StartPoint_SewerSplit( entity player )
{
	...
	CreatePuzzle1Blockers()
	thread Puzzle1Think( player )
...
void function CreatePuzzle1Blockers()
{
	// hmmmmm there seems to be a lot of repeated code around here
...
void function TryCreatePuzzle1BacktrackBlocker( bool doIt = true )
{
	if ( doIt )
	{
...
void function Puzzle1Think( entity player ) 
{
	...
	while ( true )
	{
		...
		{
			// update if player has key
			foundKey = player.GetOffhandWeapon( OFFHAND_ORDNANCE ).GetWeaponClassName() == "mp_weapon_grenade_gravity" 
			currentRepelPoint = foundKey ? P1_REPEL_POINT_BACKTRACK : P1_REPEL_GOAL_POINT 
			...
			if ( Distance( orgNoZ, currentRepelPoint ) < 250 )
			{
				...
				#if SERVER
				if ( currentRepelPoint == P1_REPEL_GOAL_POINT ) 
					SendHudMessage( player, "Requires key: gravity stars", -1, 0.6, 192, 255, 0, 255, 0.0, 5, 2 ) 
				#endif
			}
		}
...
void function KaneArena_BTMovesOutOfTheWay( entity player )
{
	...
	if ( clampedPos != null )
	{
		...
		if( randomSpots.len() > 0 )
		{
			...
			DebugDrawSphere( btSpot, 25.0, 0, 255, 0, true, 60.0 ) // UNUSED
