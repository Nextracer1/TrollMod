...
const asset RIFLE_WEAPON_MODEL			= $"models/domestic/nessy_blue_doll.mdl" //$"models/weapons/rspn101/w_rspn101.mdl"
...
const asset GRUNT_EAR_MODEL		 		= $"models/humans/grunts/mlt_grunt_rifle.mdl" //$"models/props/grunt_ear/prop_grunt_ear_animated.mdl"
...
const vector RAVINE_DEFLECT_POINT = Vector( -9690.41, 6240.56, 1026.15 )
const vector RAVINE_DEFLECT_RESPAWN = Vector( -9673.86, 5859.70, 1106.29 )
...
void function PlayerDidLoad( entity player )
{
	...
	thread RavineDeflectThink( player )
}
...
void function FamilyPhoto_Enemies()
{
	...
	entity richter = SpawnNPCByClassname( "npc_soldier", TEAM_IMC, scriptRef.GetOrigin(), <0,0,0>, "mp_weapon_droneplasma" )
	entity richterWeaponModel = CreatePropDynamic( RIFLE_WEAPON_MODEL )
	richterWeaponModel.SetParent( richter, "PROPGUN", false, 0 )
...
void function BTShootsPlayer( entity player )
{
	player.EndSignal( "OnDestroy" )
...
void function FieldPromotion_Promotion_Player( entity player )
{
	...
	ogFinalWordsSequence.thirdPersonAnim = "pt_mp_execution_attacker_steal" //"pt_wilds_OG_final_words"
	...
	ClientCommand( player, "firstperson" )
...
void function StartPoint_Battery2Path( entity player )
{
	...
	file.droneDialogArray.append( [ "diag_sp_callOut_WD752_01_01_imc_genMarder", "diag_sp_callOut_WD752_02_01_imc_genMarder" ] ) // UNUSED
	file.droneDialogArray.append( [ "diag_sp_callOut_WD752_01_01_imc_genMarder" ] )
	file.droneDialogArray.append( [ "diag_sp_callOut_WD752_03_01_imc_genMarder" ] ) // UNUSED
...
void function ShipAmbientDialogue( string speakerName = "ship_stern_speaker", string endSignal = "slot_battery2" )
{
	...
	while( true )
	{
		...
		{
			EmitSoundOnEntArray( speakerArray, "diag_sp_batteryA_WD120_01_01_mcor_sarah" ) // UNUSED
			wait RandomFloatRange(6, 12) // ADDED MYSELF
...
//	DisableMainTitanWeapon( player )
	...
	thread EnemyWave( player, "rr_drone_beam" )
	thread EnemyWave( player, "tt_smr_spectre", 1 ) // UNUSED
...
void function FinalFight( entity player )
{
	...
	if ( player.IsTitan() )  
	{
		//player.SetEjecting( true )
		ClientCommand( player, "TitanEject 3" )

	} else if ( player.GetPetTitan() != null ) 
	{
		//player.GetPetTitan().SetEjecting( true )
		player.GetPetTitan().SetHealth( 0 );
	}
...
{
	entity ogPilot = SpawnNPCByClassname( "npc_soldier", TEAM_MILITIA, <0,0,1000>, <0,0,0>, "mp_weapon_droneplasma" )
	ogWeaponModel = CreatePropDynamic( RIFLE_WEAPON_MODEL )
	ogWeaponModel.SetParent( ogPilot, "PROPGUN", false, 0 )
...
void function RavineDeflectThink( entity player )
{
	player.EndSignal( "OnDestroy" )
	...
	while ( true )
	{
		if ( Distance( player.GetOrigin(), RAVINE_DEFLECT_POINT ) < 200 )
		{
			justDeflected = true

			if ( player.GetVelocity().y > 385 )
			{
			...
			{
				player.SetOrigin( RAVINE_DEFLECT_RESPAWN )
