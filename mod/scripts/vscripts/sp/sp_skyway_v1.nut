...
//global function Test_InitRandomEntitySpawer
...
void function CodeCallback_MapInit()
{
	...
	RegisterSignal( "SPLoadoutMenuClosed" ) // UNUSED fix attempt
	...
	//AddClientCommandCallback( "Test_SpawnRandomEntities", Test_InitRandomEntitySpawer )
...
void function EntitiesDidLoad()
{
	...
	// EmitSoundOnEntity( file.core, "skyway_emit_foldweapon_normalstate_loop" ) // interesting
...
void function WaitForBTThrowPlayerImpact( entity player, entity scriptRef )
{
	...
	// file.statusEffect = StatusEffect_AddEndless( player, eStatusEffect.emp, 0.1 ) // interesting
...
void function BTTortureRoom( entity scriptRef, entity player, string animation = "BT_torture_intro" )
{
	...
	bt.Hide()
	bt = titan
	// array<string> bodyGroups = [ // interesting
	// "hip",
	// "front",
	// "right_arm",
	// "hatch"
	...
	// AddAnimEvent( bt, "dof_bt_head", FocusOnBTHead ) // interesting
	// AddAnimEvent( bt, "dof_bt_cockpit", FocusOnBTCockpit )
	// AddAnimEvent( bt, "dof_bt_r_hand", FocusOnBTRightHand )
	// AddAnimEvent( bt, "bt_show_core_in_cockpit", ShowCoreInCockpit )
	// AddAnimEvent( bt, "bt_destroy_core_in_cockpit", DestroyCoreInCockpit )
...
{
	// int attachID = slone.LookupAttachment( "HAND_L" ) // interesting
...
void function Init_BTEyeCase( entity player, entity scriptRef )
{
	...
	// thread PlayAnimTeleport( dataKnife, "data_knife_torture_cache_pickup", moverPickup ) // interesting
...
void function StartRingRotation()
{
	...
	//outer.NonPhysicsRotate( outerForward, 75 ) // interesting
...
entity function CreateCoreEnergy( vector origin )
{
	...
	// PlayLoopFXOnEntity( FX_CORE_FLARE, coreEnergy ) // interesting
...
void function SP_Run_Dialogue( entity player )
{
	...
	// wait 0.5
	// waitthread PlayDialogue( "SP_RUN_RADIO_1", player ) // interesting
	wait 0.5
...
void function BombardDialoge2( entity player )
{
	...
	// waitthread PlayDialogue( "TITAN_HILL_A_1", player ) // interesting
	// wait 0.1
...
//	thread BombardTargetOnFlag_ClusterMortar( milShip1_starboard_gun_01 ,envTarget_01, "bombardment_env_target_01", 2.5 ) // interesting
...
void function TitanCrouchHint( entity player )
{
	...
	while( 1 )
	{
		...
		if ( !hintGiven )
		{
			// waitthread PlayDialogue( "SMASH_HALL_1", player ) // interesting
			// wait 0.5
...
void function PulldownLoopThink( entity injector, entity player, entity slone, entity mover, array<entity> spawnerArray )
{
	...
	if ( !Flag( "InjectorRoomDone" ) )
	{
		...
		// player.SnapEyeAngles( < -12.153360, 160.297989, -15.030890 > ) // interesting
...
void function SloneFightPhases( entity player, entity slone )
{
	...
	// Phase 1
	// SpawnFromSpawnerArray( GetEntArrayByScriptName( "slone_fight_phase_1" ) ) // interesting
	...
	// {
	// 	thread TriggerSpawnSpawner( ent ) // interesting
	// }
...
void function ManagePhases( entity player )
{
	...
	// thread LightstripSounds() // interesting
	...
	if ( !Flag( "StoppingInjector" ) )
	{
		...
		// delaythread( 2.0 ) ScreenFadeToBlackForever( player ) // INTERESTING
	}
}
...
void function SloneFightMobs( entity player, entity slone, array<string> spawnerNames, bool viewSpawn, string line1, string line2, string line3 )
{
	...
	// SetStanceKneel( soul ) // interesting
	...
	// SetStanceStanding( soul ) // interesting 2 ^
...
void function BliskFarewellRadioPlay( entity player )
{
	...
	// waitthread PlayDialogue( "BLISK_FAREWELL_b_2", player ) // interesting
	// wait 3.8
...
void function FakeBliskFootstep( entity player, float dist, float intensity )
{
	...
	// EmitSoundAtPosition( TEAM_ANY, player.GetOrigin() + fwd*dist, "exo_stomp" ) // interesting
...
void function PlayerCardThrow( entity scriptRef, entity player )
{
	...
	// thread BTSacrificeDialogue_BTMemory( player ) // interesting
...
void function SP_BTSacrificeThread( entity player )
{
	...
	CheckPoint_Silent()
	// waitthread Init_BTLoadingBayClimb( "#SKYWAY_HINT_ENTER_INJECTOR" , "#SKYWAY_HINT_ENTER_INJECTOR_PC" ) // interesting
...
void function BTSacrificeDialogue( entity player )
{
	...
	// waitthread PlayDialogue( "BT_SACRIFICE_6", player ) // interesting ^ 2
	// wait 0.5
...
void function EjectionSequence( entity player )
{
	...
	// ScreenFadeToBlackForever( player, 0.2 ) // interesting
	// wait 0.2
...
void function BTThrowsPlayer( entity player, entity mover )
{
	...
	// GibBodyPart( titan, "left_arm" ) // interesting
...
void function BTThrowsPlayer_Player( entity player, entity mover )
{
	...
	// EmitSoundOnEntity( titan, "titan_nuclear_death_charge" ) // interesting
...
void function WarpInThread( entity player, entity dropship, entity idleMover, entity dropshipRef )
{
	...
	// entity mover = CreateScriptMover() // interesting
...
void function RedirectPlayerForceToTarget( string triggerFlag, entity target, float stopTime )
{
	...
	//thread BlackholePlayerToShip( player, target, 1 ) // interesting
...
void function SP_ExplodingPlanetThread( entity player )
{
	...
	// entity idleMover = CreateScriptMover() // interesting
	// idleMover.SetParent( dropship, "RESCUE", false, 0.0 )
	// int id = dropship.LookupAttachment( "RESCUE" )
	// vector angles = dropship.GetAttachmentAngles( id )
	// vector fwd = AnglesToForward( angles )
...
void function AnimateStuff( entity player, entity planet, entity dropship, entity mover )
{
	...
	// thread PlayAnimTeleport( dropship, "dropship_planet_explosion_slow_2", mover ) // interesting
